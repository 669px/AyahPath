...
SURAH_NAMES_EN = {1: 'Al-Fatihah', 2: 'Al-Baqarah', 3: "Ali 'Imran", 4: 'An-Nisa', 8: 'Al-Anfal', 9: 'At-Tawbah', 13: "Ar-Ra'd", 14: 'Ibrahim', 16: 'An-Nahl', 17: 'Al-Isra', 31: 'Luqman', 39: 'Az-Zumar', 41: 'Fussilat', 42: 'Ash-Shuraa', 57: 'Al-Hadid', 65: 'At-Talaq', 94: 'Ash-Sharh', 113: 'Al-Falaq'}
TRANSLATION_NAMES = {131: 'Dr. Mustafa Khattab (The Clear Quran)', 97: 'Maulana Fateh Muhammad Jalandhari', 84: 'Hilali & Khan', 136: 'Montada Islamic Foundation', 33: 'Indonesian Ministry of Religious Affairs', 20: 'Taisirul Quran', 77: 'Diyanet Isleri', 54: 'Farook Khan & Nadwi'}
SURAH_NAMES_AR = {1: 'الفاتحة', 2: 'البقرة', 3: 'آل عمران', 4: 'النساء', 8: 'الأنفال', 9: 'التوبة', 13: 'الرعد', 14: 'إبراهيم', 16: 'النحل', 17: 'الإسراء', 31: 'لقمان', 39: 'الزمر', 41: 'فصلت', 42: 'الشورى', 57: 'الحديد', 65: 'الطلاق', 94: 'الشرح', 113: 'الفلق'}
...
def _translator_name(resource_id):
    rid = _safe_int(resource_id)
    if rid is None:
        return ''
    return TRANSLATION_NAMES.get(rid, f'Translation {rid}')
...
def _map_verse_for_ui(item, requested_translation_id='131'):
    ...
    return {'chapter': chapter, 'verse': verse, 'verse_key': verse_key or f'{chapter}:{verse}', 'arabic': str(item.get('text_uthmani') or item.get('text_uthmani_simple') or item.get('text_imlaei') or ''), 'translation': primary_translation, 'secondary_translation': secondary_translation, 'translator_name': _translator_name(primary_id), 'secondary_translator_name': _translator_name(requested_id) if secondary_translation else '', 'surah_name': str(item.get('chapter', {}).get('name_simple') or SURAH_NAMES_EN.get(chapter, f'Surah {chapter}')), 'surah_name_ar': str(item.get('chapter', {}).get('name_arabic') or SURAH_NAMES_AR.get(chapter, '')), 'audio': audio_url, 'audio_url': audio_url, 'success': True}
...
def _fetch_official_verse(verse_key, translation_id):
    params = {'language': 'en', 'words': 'false', 'translations': _translation_ids(translation_id), 'audio': Config.QURAN_DEFAULT_RECITATION, 'fields': ','.join(['text_uthmani', 'text_uthmani_simple', 'text_imlaei', 'chapter_id', 'verse_number', 'verse_key', 'juz_number', 'hizb_number', 'page_number'])}
    ...
    for category in virtual_get_categories():
        ...
        for row in rows:
            ...
            if chapter_id is None:
                continue
            item = chapters.setdefault(chapter_id, {'id': chapter_id, 'chapter_number': chapter_id, 'name_simple': SURAH_NAMES_EN.get(chapter_id, f'Surah {chapter_id}'), 'name_arabic': SURAH_NAMES_AR.get(chapter_id, ''), 'verses_count': 0})
            item['verses_count'] += 1
...
def get_tafsir(chapter, verse):
    ...
    candidates = [(f'tafsirs/{tafsir_id}', {'verse_key': verse_key}), (f'tafsirs/{tafsir_id}/by_ayah/{verse_key}', None), (f'tafsirs/by_ayah/{verse_key}', {'tafsirs': tafsir_id})]
