...
KEYWORD_CATEGORY_MAP = {'stress': ('stress', 'anxiety', 'sad', 'down', 'worried', 'worry', 'fear', 'panic', 'overwhelmed', 'depressed'), 'anger': ('angry', 'anger', 'furious', 'mad', 'rage'), 'jealousy': ('jealous', 'envy', 'envious', 'resent'), 'gratitude': ('grateful', 'gratitude', 'thankful', 'blessing'), 'forgiveness': ('forgive', 'forgiveness', 'grudge', 'resentment'), 'trust_in_allah': ('trust', 'tawakkul', 'uncertain', 'future', 'rely', 'reliance'), 'patience': ('patient', 'patience', 'sabr', 'waiting', 'hardship', 'trial'), 'charity': ('charity', 'donate', 'giving', 'sadaqah', 'helping'), 'lying': ('lie', 'lying', 'dishonest', 'truth'), 'arrogance': ('pride', 'arrogant', 'ego', 'boast')}
SUPPORTIVE_CATEGORY_MAP = {'stress': 'trust_in_allah', 'anger': 'forgiveness', 'jealousy': 'gratitude', 'lying': 'patience', 'arrogance': 'gratitude', 'patience': 'patience', 'gratitude': 'gratitude', 'forgiveness': 'forgiveness', 'trust_in_allah': 'trust_in_allah', 'charity': 'charity'}
...
def _build_memory_summary(user_id):
    ...
    for item in reflections[-4:]:
        reflection_lines.append(f'- {item.get('timestamp', '')}: {item.get('assigned_category', 'unknown')} | {item.get('scenario', '')[:160]}')
    activity_lines = []
    for item in activities[-6:]:
        activity_lines.append(f'- {item.get('timestamp', '')}: {item.get('action', '')} | {item.get('details', '')[:100]}')
    return '\n'.join(['Recent reflections:', *(reflection_lines or ['- No saved reflections yet.']), 'Recent activity:', *(activity_lines or ['- No recent activity yet.'])])
...
def _make_openrouter_call(prompt, is_json=False):
    ...
    headers = {'Authorization': f'Bearer {Config.OPENROUTER_API_KEY}', 'Content-Type': 'application/json', 'HTTP-Referer': Config.APP_URL, 'X-Title': 'AyahPath'}
    payload = {'model': Config.AI_MODEL, 'messages': [{'role': 'user', 'content': prompt}], 'temperature': 0.3}
    if is_json:
...
def assign_category(scenario, user_id='anonymous_user'):
    ...
    prompt = f'''You are categorising a user's latest reflection for an Islamic guidance app.\n{memory}\nReflection: "{scenario}"\nOptions: {', '.join(categories)}.\nRespond with ONLY the category name.'''
...
def generate_guidance(scenario, category, verse_text, tafsir_text, user_id='anonymous_user'):
    ...
    fallback = {'dunya_impact': 'Focus on patience and practical steps in your daily affairs.', 'akhirah_impact': "Trust in Allah's plan and seek reward in the hereafter.", 'better_choice': 'Take a moment for prayer and reflection today.', 'why_this_verse': "This verse reminds us of Allah's presence in all situations."}
...
def get_personalized_ayah_plan(user_id='anonymous_user'):
    ...
    return {'category': category, 'scenario': scenario, 'reason': reason, 'top_category': top_category}
