...
-- This is the source of truth for all prayer tracking
CREATE TABLE IF NOT EXISTS prayer_logs (
    ...
    completed   INTEGER NOT NULL DEFAULT 1,  -- 1 = prayed, 0 = not prayed
    checked_at  TEXT    NOT NULL,  -- ISO timestamp when toggled on
    created_at  TEXT    NOT NULL DEFAULT (datetime('now')),
    updated_at  TEXT    NOT NULL DEFAULT (datetime('now')),

    -- Each user can only have one entry per prayer per day
...
-- Updated on every toggle for fast reads
CREATE TABLE IF NOT EXISTS prayer_daily_summary (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id         TEXT    NOT NULL DEFAULT 'anonymous_user',
    prayer_date     TEXT    NOT NULL,  -- ISO date: YYYY-MM-DD
    completed_count INTEGER NOT NULL DEFAULT 0,  -- 0-5
    total_prayers   INTEGER NOT NULL DEFAULT 5,
    is_complete     INTEGER NOT NULL DEFAULT 0,  -- 1 if all 5 done
    updated_at      TEXT    NOT NULL DEFAULT (datetime('now')),
...
-- Prayer streak cache: running stats per user
-- Recalculated on each toggle for correctness
...
def toggle_prayer(user_id, prayer_date_str, prayer_id):
    ...
    with get_db() as conn:
        ...
        if row and row['completed'] == 1:
            ...
            # Does not exist → insert as checked
            conn.execute(
...
def get_prayer_stats(user_id):
    ...
    with get_db() as conn:
        ...
        # Week completion: count prayers in the last 7 days
        ...
        week_total = 7 * 5  # 7 days × 5 prayers
...
def _recalculate_streak(conn, user_id):
    ...
    """
    # Get all complete days ordered descending
    rows = conn.execute(
    ...
    if check_date.isoformat() in complete_dates:
        ...
        # Check if yesterday is there (streak might not include today yet)
        check_date -= timedelta(days=0)  # start from today going back
        # If today is not complete, check backwards from yesterday
    ...
    for _ in range(365):
        if check_date.isoformat() in complete_dates:
            if streak == 0:
                streak = 1  # Starting from yesterday
            else:
                streak += 1
            check_date -= timedelta(days=1)
        else:
