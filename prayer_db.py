...
def toggle_prayer(user_id, prayer_date_str, prayer_id):
    ...
    with get_db() as conn:
        row = conn.execute('SELECT id, completed FROM prayer_logs WHERE user_id=? AND prayer_date=? AND prayer_id=?', (user_id, prayer_date_str, prayer_id)).fetchone()
        now = datetime.utcnow().isoformat()
        if row and row['completed'] == 1:
            conn.execute('DELETE FROM prayer_logs WHERE id=?', (row['id'],))
            new_state = False
        elif row and row['completed'] == 0:
            conn.execute('UPDATE prayer_logs SET completed=1, checked_at=?, updated_at=? WHERE id=?', (now, now, row['id']))
            new_state = True
        else:
            conn.execute('INSERT INTO prayer_logs (user_id, prayer_date, prayer_id, completed, checked_at, created_at, updated_at) VALUES (?,?,?,1,?,?,?)', (user_id, prayer_date_str, prayer_id, now, now, now))
            new_state = True
...
def get_prayers_for_date(user_id, prayer_date_str):
    with get_db() as conn:
        rows = conn.execute('SELECT prayer_id, completed FROM prayer_logs WHERE user_id=? AND prayer_date=? AND completed=1', (user_id, prayer_date_str)).fetchall()
...
def get_week_summary(user_id, reference_date_str=None):
    ...
    with get_db() as conn:
        ...
        rows = conn.execute(f'SELECT prayer_date, completed_count, is_complete FROM prayer_daily_summary WHERE user_id=? AND prayer_date IN ({placeholders})', [user_id] + dates).fetchall()
    summary_map = {row['prayer_date']: dict(row) for row in rows}
    today_str = date.today().isoformat()
    result = []
    for i, d in enumerate(dates):
        s = summary_map.get(d, {})
        result.append({'date': d, 'day_label': day_labels[i], 'completed_count': s.get('completed_count', 0), 'is_complete': bool(s.get('is_complete', 0)), 'is_today': d == today_str})
    return result

def get_prayer_stats(user_id):
    with get_db() as conn:
        streak_row = conn.execute('SELECT * FROM prayer_streaks WHERE user_id=?', (user_id,)).fetchone()
        week_start = (date.today() - timedelta(days=6)).isoformat()
        week_row = conn.execute('SELECT COALESCE(SUM(completed_count), 0) as week_prayed FROM prayer_daily_summary WHERE user_id=? AND prayer_date >= ?', (user_id, week_start)).fetchone()
        week_prayed = week_row['week_prayed'] if week_row else 0
        week_total = 7 * 5
    return {'current_streak': streak_row['current_streak'] if streak_row else 0, 'longest_streak': streak_row['longest_streak'] if streak_row else 0, 'total_prayers': streak_row['total_prayers_logged'] if streak_row else 0, 'total_complete_days': streak_row['total_complete_days'] if streak_row else 0, 'week_completion_pct': round(week_prayed / week_total * 100) if week_total > 0 else 0}
...
def _update_daily_summary(conn, user_id, prayer_date_str):
    row = conn.execute('SELECT COUNT(*) as cnt FROM prayer_logs WHERE user_id=? AND prayer_date=? AND completed=1', (user_id, prayer_date_str)).fetchone()
...
def _recalculate_streak(conn, user_id):
    rows = conn.execute('SELECT prayer_date FROM prayer_daily_summary WHERE user_id=? AND is_complete=1 ORDER BY prayer_date DESC', (user_id,)).fetchall()
    complete_dates = set((row['prayer_date'] for row in rows))
    total_row = conn.execute('SELECT COUNT(*) as cnt FROM prayer_logs WHERE user_id=? AND completed=1', (user_id,)).fetchone()
